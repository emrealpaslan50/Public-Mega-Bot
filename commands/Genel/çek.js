module.exports = {
    config: {
        ...
        usage: "<@Member || ID>",
        category: "Genel",
        description: "Etiketlediğiniz üye sesteyse yanına çekersiniz.",
        accessableby: "Herkes"
    },
    run: async(bot, message, args) => {
        ...
        if (!args[0]) {
            ...
            message.channel.send({
                embed: {
                    color: embişx.noEmbed.color,
                    title: embişx.noEmbed.title,
                    description: `${bot.emojis.cache.get(em.no) || "❌"} **Yanına çekilcek üyeyi etiketle veya ID gir!**`,
                    timestamp: new Date(),
                    footer: {
                        icon_url: message.author.displayAvatarURL({ dynamic: true }),
                        text: embişx.noEmbed.footer
        ...
        message.channel.send(`${member}, ${message.author} **ses kanalında yanına çekmek istiyor!**`)
            .then(a => {
                a.react(em.yes || '✅');
                a.react(em.no || '❌');
